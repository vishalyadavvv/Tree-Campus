...
const LessonView = () => {
  ...
  // Auto-mark lesson as complete when it loads
  useEffect(() => {
    if (lesson && !isCompleted && !loading) {
      // Small delay to ensure they actually "opened" it and it's not just a rapid click-through
      const timer = setTimeout(() => {
        markAsComplete(false); // pass false to suppress popup
      }, 2000);
  ...
  const markAsComplete = async (showPopup = true) => {
    try {
      if (allLessons.find(l => l._id === lessonId)?.isCompleted) return;
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mt-3">
                ...
                <div className="space-y-2">
                  <div className="flex justify-center">
                    {/* Manual 'Mark as Complete' removed to favor automatic completion */}
                  </div>
